...
class VOT(nn.Module):
     def __init__(self, *, image_size, patch_size, ac_patch_size,
                          pad, dim, depth, heads, mlp_dim, pool = 'cls', channels = 1, dim_head = 32, dropout = 0.15, emb_dropout = 0):
         super().__init__()
         ...
         self.transformer1 = Transformer(dim, 12, 8, 32, 2048, 0.15)
         self.transformer2 = Transformer(dim, 6, 4, 32, 1024, 0.15)
     ...
     def forward(self, x,  mask = None):
         ...
         with torch.cuda.amp.autocast(enabled=False):
             ...
             x = self.dropout(x)
             x1 = self.transformer1(x, mask)
             x2 = self.transformer2(x, mask)
             #x3 = self.transformer3(x, mask)
             x=self.alpha*x1+(1-self.gama)*x2
             #x=self.alpha*x1+self.gama*x2+(1-self.alpha-self.gama)*x3
